...
class Lexer
  ...
  def error(msg=nil)
    puts "error in state %s, position: %s" % [@state, curr_input.offset]
  ...
  def lex_lit_str_escape
    ...
    else; error('unknown escape sequence: $%s' % [@current])
    end
  end
  ...
  def lex_main
    ...
    when -1; 
      curr_input.first_ln = curr_input.curr_ln; complete(:EOF, 0)
    else; error
    end
  end
